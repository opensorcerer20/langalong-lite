/* Tsumiki — ported from original Claude Design file `Sentence Builder.dc.html`. The content, the tile-bank
...
function bankFor(i) {
  ...
  (item.alts || []).forEach((alt) => {
    ...
    while (rest.length) {
      let hit = null;
      vocab.forEach((t) => {
        if (rest.indexOf(t[0]) === 0 && (!hit || t[0].length > hit[0].length))
          hit = t;
      });
      if (!hit) break;
      if (!used[hit[0]]) {
  ...
  const pool = GRAMMAR.concat(SCENARIOS[state.sc].words).filter(
    (t) => !used[t[0]],
  );
  let n = 0;
...
function render() {
  ...
  $("headerLabel").textContent =
    state.screen === "home"
      ? "Japanese · beginner"
      : SCENARIOS[state.sc].name +
        " · " +
        SCENARIOS[state.sc].kicker.replace("Set ", "");
  ...
  $("promptKicker").textContent =
    "Say this in Japanese — item " + (state.i + 1) + " of " + total;
  ...
  answerLine.replaceChildren(
    ...
    ...Array.from(
      { length: Math.max(0, item.ans.length - state.placed.length) },
      () => {
  ...
  $("status").textContent =
    state.status === "wrong"
      ? state.misses >= NOTE_AFTER_MISSES
