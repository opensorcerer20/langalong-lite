...
- **The exercise modes and the scenario flow.** Decided, and under way: vocabulary, particle practice and verb conjugation as their own exercises, then chained per situation as vocabulary → conjugation → particles → sentence building with a success screen between each. Two phases are done. Phase 0 was invisible — `appReducer` no longer knows what a sentence is, every attempt records which exercise and situation it came from, and every sentence is tagged with the particles and conjugation patterns it teaches. Phase 1 added the **vocabulary exercise** and the machinery the rest will reuse: a `Question` shape, one compiler per exercise in `src/lib/questions/`, a shared `ExerciseScreen`, and a chip under each situation on the home screen to start one. Particles and conjugation are each a compiler and a chip from here. Jumping straight to one exercise stays possible by design rather than being replaced by the flow.
- **Deciding whether the two exercise paths should merge.** The sentence drill kept its own screen and hook rather than being folded into the `Question` pipeline, because it was working and Phase 1 should not have risked it. It leaves two of several things — two hooks, two screens, two copies of `tap`/`untap`/`next`/`restart`. Worth revisiting once particles and conjugation exist, when the size of the overlap is visible rather than guessed at; the test is how many fields a merged `Question` would carry that only one mode uses.
